...
class InputWindow(QtWidgets.QWidget):
    ...
    def set(self):
        try:
            ...
            self.parent.title.setText(f'{title} {a["サブタイトル/コメント▲▼"]}')
            self.parent.from_search = True
        except:
            self.parent.from_search = False
            self.parent.comments = []
            self.parent.duration = 0
            self.parent.title.setText("")
